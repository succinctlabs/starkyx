...
mod tests {
    ...
    fn test_ed25519_add() {
        ...
        let nb_rows = 2u64.pow(16) as usize;
        let (handle, generator) = trace::<F, Cubic, D>(nb_rows);
        for i in 0..256 {
            ...
            rayon::spawn(move || {
                ...
                for j in 0..256 {
                    ...
                    let r = handle
                        .write_ed25519_add(256 * i + j, &p_int, &q_int, ed25519_add_gadget.clone())
                        .unwrap();
                    assert_eq!(r, r_exp);
                }
            })
        ...
        let proof = prove::<F, C, S, ArithmeticGenerator<F, Cubic, D>, D, 2>(
            stark.clone(),
            &config,
            generator,
            nb_rows,
    ...
    impl<F: RichField + Extendable<D>, const D: usize> StarkParameters<F, D> for EdDoubleTest {
        ...
        const NUM_FREE_COLUMNS: usize = 1160;
        type Instruction = InstructionSet<Ed25519BaseField>;
    }
    ...
    fn test_ed_double_row() {
        ...
        let (handle, generator) = trace::<F, Cubic, D>(num_rows);
