...
impl MulDLayout {
    #[inline]
    pub const fn new(a: Register, output: Register, witness: Register) -> Self {
...
impl<F: RichField + Extendable<D>, const D: usize> ArithmeticParser<F, D> {
    ...
    pub fn muld_trace(a: BigUint) -> (Vec<F>, BigUint) {
        ...
        let carry = (&a * &d - &result) / &p;
        debug_assert!(carry < p);
        debug_assert_eq!(&carry * &p, &a * &d - &result);
        ...
        let vanishing_poly = &p_a * &p_d - &p_result - &p_carry * &p_p;
    ...
    pub fn muld_packed_generic_constraints<
        FE,
        P,
        const D2: usize,
        const COLUMNS: usize,
        ...
        P: PackedField<Scalar = FE>,
    {
        ...
        let d = D_iter::<FE>().map(P::from).collect::<Vec<_>>();
        ...
        let ad = PolynomialOps::mul(a, &d);
        let ad_minus_output = PolynomialOps::sub(&ad, output);
        let p_limbs = Polynomial::<FE>::from_iter(P_iter());
        let mul_times_carry = PolynomialOps::scalar_poly_mul(carry, p_limbs.as_slice());
        let vanishing_poly = PolynomialOps::sub(&ad_minus_output, &mul_times_carry);
    ...
    pub fn muld_ext_constraints<const COLUMNS: usize, const PUBLIC_INPUTS: usize>(
        layout: MulDLayout,
        ...
        let d = PolynomialGadget::constant_extension(builder, &D_iter().collect::<Vec<_>>()[..]);
        ...
        // Construct the expected vanishing polynmial
        let ad = PolynomialGadget::mul_extension(builder, a, &d);
        let ad_minus_output = PolynomialGadget::sub_extension(builder, &ad, output);
        let p_limbs =
        ...
        let vanishing_poly =
            PolynomialGadget::sub_extension(builder, &ad_minus_output, &mul_times_carry);
...
mod tests {
    ...
    pub struct MulDLayoutCircuit;

    const LAYOUT: MulDLayout = MulDLayout::new(
        Register::Local(0, N_LIMBS),
        Register::Local(N_LIMBS, N_LIMBS),
        Register::Local(2 * N_LIMBS, NUM_CARRY_LIMBS + 2 * NUM_WITNESS_LIMBS),
    );

    const INLAYOUT: WriteInputLayout = WriteInputLayout::new(Register::Local(0, N_LIMBS));

    impl<F: RichField + Extendable<D>, const D: usize> EmulatedCircuitLayout<F, D, 2>
        for MulDLayoutCircuit
    {
        ...
        const OPERATIONS: [EpOpcodewithInputLayout; 2] = [
            EpOpcodewithInputLayout::Ep(EdOpcodeLayout::MULD(LAYOUT)),
            EpOpcodewithInputLayout::Input(INLAYOUT),
        ];
    }

    #[derive(Debug, Clone)]
    pub struct MulDInstruction {
        pub a: BigUint,
    }

    impl MulDInstruction {
    ...
    #[test]
    fn test_arithmetic_stark_muld() {
        const D: usize = 2;
        type C = PoseidonGoldilocksConfig;
        type F = <C as GenericConfig<D>>::F;
        type S = ArithmeticStark<MulDLayoutCircuit, 2, F, D>;
        ...
        for _ in 0..num_rows {
            ...
            quad_operations.push(MulDInstruction::new(a));
        }
