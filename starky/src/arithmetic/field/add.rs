...
pub struct FpAdd<P: FieldParameters<N_LIMBS>, const N_LIMBS: usize> {
...
impl<L: ChipParameters<F, D>, F: RichField + Extendable<D>, const D: usize> ChipBuilder<L, F, D> {
    pub fn fpadd<P: FieldParameters<N>, const N: usize>(
        &mut self,
        a: &FieldRegister<P, N>,
        b: &FieldRegister<P, N>,
...
impl<P: FieldParameters<N_LIMBS>, const N_LIMBS: usize> FpAdd<P, N_LIMBS> {
...
impl<F: RichField + Extendable<D>, const D: usize, const N: usize, FP: FieldParameters<N>>
    Instruction<F, D> for FpAdd<FP, N>
{
...
impl<F: RichField + Extendable<D>, const D: usize> TraceHandle<F, D> {
    pub fn write_fpadd<P: FieldParameters<N_LIMBS>, const N_LIMBS: usize>(
        &self,
        row_index: usize,
        a_int: &BigUint,
        b_int: &BigUint,
        instruction: FpAdd<P, N_LIMBS>,
    ) -> Result<BigUint> {
        let (row, result) = FpAdd::<P, N_LIMBS>::trace_row::<F, D>(a_int, b_int);
...
mod tests {
    ...
    impl<F: RichField + Extendable<D>, const D: usize> ChipParameters<F, D> for FpAddTest {
        const NUM_ARITHMETIC_COLUMNS: usize = FpAdd::<Fp25519Param, 16>::num_add_columns();
        const NUM_FREE_COLUMNS: usize = 0;

        type Instruction = FpAdd<Fp25519Param, 16>;
    }
