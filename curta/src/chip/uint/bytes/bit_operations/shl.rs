        ...
        L::Instruction: From<SelectInstruction<BitRegister>>,
    {
        ...
        for (k, bit) in b.into_iter().enumerate() {
            ...
            for i in num_shift_bits..n {
                self.set_select(&bit, &temp.get(i-num_shift_bits), &temp.get(i), &res.get(i));
            }
            temp = res;
        }
    }
...
pub mod tests {
    ...
    #[derive(Debug, Clone)]
    pub struct ShlTest<const N: usize, const M: usize>;

    impl<const N: usize, const M: usize> const AirParameters for ShlTest<N, M> {
