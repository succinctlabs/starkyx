...
impl<F: PrimeField64> Instruction<F> for ByteOperation<ByteRegister> {
    ...
    fn write(&self, writer: &TraceWriter<F>, row_index: usize) {
        ...
        match self {
            ByteOperation::And(a, b, c) => {
                ...
                let c_val = as_field(a_val & b_val);
                writer.write(c, &c_val, row_index);
            }
            ByteOperation::Xor(a, b, c) => {
                let a_val = from_field(writer.read(a, row_index));
                let b_val = from_field(writer.read(b, row_index));
            ...
            ByteOperation::Rot(a, b, c) => {
                let a_val = from_field(writer.read(a, row_index));
                let b_val = from_field(writer.read(b, row_index));
                let c_val = as_field(a_val.rotate_right(b_val as u32));
                writer.write(c, &c_val, row_index);
            }
            ByteOperation::Not(a, b) => {
                let a_val = from_field(writer.read(a, row_index));
                let b_val = as_field(!a_val);
                writer.write(b, &b_val, row_index);
            }
